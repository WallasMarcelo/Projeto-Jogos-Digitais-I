    ...
    void FixedUpdate()
    {
       ...
       if(timeAttack <=0){
            ...
            if(Input.GetKeyDown(KeyCode.Z)){
                animator.SetTrigger("isAttacking"); // Inicio animação
                timeAttack = startTimeSttack; // Tempo para colocar no Unity
            }

        }else{
                timeAttack -= Time.deltaTime; // Decrescer em tempo real
                animator.SetTrigger("isAttacking"); // termino animação
        }
